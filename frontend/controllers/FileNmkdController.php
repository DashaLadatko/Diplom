...
class FilenmkdController extends Controller
{
    ...
    public function actionAuditnmkd()
    {
        if (Yii::$app->user->identity->role !== User::ROLE_ADMINNMKD) {
            throw new ForbiddenHttpException('Access denied');
        }
        ...
        $rows = Yii::$app->db->createCommand('SELECT user_id, file_nmkd.name FROM `file_nmkd` '
            .'WHERE DATEDIFF(NOW(),FROM_UNIXTIME( created_at))>10 AND total=1 limit 1')
            ->queryAll();
        foreach ($rows as $row) {
            Yii::$app->mailer->compose()
                ->setFrom(Yii::$app->params['adminEmail'])
                ->setTo(User::findOne($row['user_id'])->email)
                ->setTextBody(' Шановний ' . User::findOne($row['user_id'])->getFullName()
...
//->send()
        ...
        $sends = Yii::$app->db->createCommand('SELECT user_id FROM `file_nmkd` '
            .'WHERE signature=\'не завантажено\' AND DATEDIFF(NOW(),FROM_UNIXTIME( created_at))>5 ')->queryAll();
